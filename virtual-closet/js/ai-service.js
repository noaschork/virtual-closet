...
export class AIService {
    ...
    async generateOutfit(items, preferences, options = {}) {
        const { occasion, weather } = options;
        ...
        const itemsDescription = items.map(item =>
            `${item.type}: ${item.color} ${item.silhouette} (ID: ${item.id})`
        ).join('\n');
...
Instructions:
1. Select items that work well together (1 top OR 1 dress, 1 bottom if not dress, 1 outerwear if needed, 1 shoes, accessories if appropriate)
2. Consider color coordination, style harmony, and appropriateness for the occasion and weather
3. Provide the outfit as a JSON object with this structure:
{
  "items": [
...
Return ONLY the JSON object, no additional text.`;
        ...
        const response = await this.makeRequest([
            { role: 'system', content: 'You are a professional fashion stylist who creates cohesive, stylish outfit combinations.' },
...
4. Avoid repeating the same items on consecutive days when possible
5. Return a JSON array with 7 outfit objects:
                ...
                messages: [
                    {
                        ...
                        content: [
                            {
                                type: 'text',
                                text: 'Analyze this clothing item and return a JSON object with: type (tops/bottoms/dresses/outerwear/shoes/accessories), color (black/white/gray/brown/beige/red/pink/orange/yellow/green/blue/purple/multicolor), and silhouette (fitted/loose/oversized/flowy/structured). Return ONLY the JSON object.'
                            },
                            {
                                type: 'image_url',
