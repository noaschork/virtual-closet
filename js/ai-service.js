...
2. IMPORTANT outfit rules:
   ...
   - If you choose a SKIRT, you MUST include a TOP and/or SWEATER (skirts are bottoms)
   - NEVER pair black knee-high boots with light colored dresses
   - NEVER pair boots with white bottoms or white dresses
...
5. WEATHER-APPROPRIATE CHOICES:
   - AVOID black bottoms in hot/warm weather (too heavy and heat-absorbing)
   - AVOID long sleeves in hot weather (uncomfortable and impractical)
   - AVOID boots in hot/warm weather (too warm for the temperature)
   - AVOID heels for casual occasions (sneakers, flats, or sandals are more appropriate)
   - NO sweaters in hot weather (too warm)
   - Sandals ONLY in hot weather (not appropriate for cold/cool weather)
   - NO white bottoms in cold weather (impractical and seasonally inappropriate)
6. Consider color coordination, style harmony, and appropriateness for the occasion and weather
7. BE CREATIVE - try different combinations each time, don't repeat the same items
8. Provide the outfit as a JSON object with this structure:
{
  "items": [
...
4. STRICT RULES:
   ...
   - AVOID black bottoms when temperature is above 70°F
   - AVOID long sleeves when temperature is above 80°F
   - AVOID boots when temperature is above 70°F
   - AVOID heels for casual events (use sneakers, flats, or sandals instead)
   - NO sweaters when temperature is above 80°F
   - Sandals ONLY when temperature is above 75°F
   - NO white bottoms when temperature is below 50°F
...
Return ONLY the JSON array, no additional text.`;
    ...
    // Generate vision board outfits (30 complete outfits)
    async generateVisionBoardOutfits(items, boardName) {
        const itemsDescription = items.map(item =>
            `${item.type}: ${item.color} ${item.silhouette}, ${item.season || 'all-season'} (ID: ${item.id})`
        ).join('\n');

        const prompt = `You are a professional stylist creating a complete outfit capsule for "${boardName}".

Available wardrobe items:
${itemsDescription}

Create 30 complete outfits with this breakdown:
- 18 casual outfits (everyday wear, running errands, coffee dates)
- 10 date night/dinner outfits (nice restaurants, evening events)
- 2 gala/party outfits (formal events, special occasions)

IMPORTANT RULES:
- Each outfit must be complete (top/dress + bottom + shoes, optionally accessories/outerwear)
- Items can be repeated in up to 2 different outfits (allows versatility)
- Follow all outfit rules: no dress+top+bottom, season matching, weather appropriate
- SKIRTS must be paired with a top and/or sweater (skirts are bottoms, not standalone)
- NO heels in casual outfits (use sneakers, flats, or sandals)
- Casual outfits should be comfortable and practical
- Date/dinner outfits should be polished and put-together
- Date/dinner outfits: ONLY heeled black boots allowed (no other heels)
- NEVER pair black knee-high boots with light colored dresses
- NEVER pair boots with white bottoms or white dresses
- Gala outfit #1: Floor-length black tie with heels, NO outerwear
- Gala outfit #2: Cocktail/midi/mini dress with heels, NO outerwear

Return JSON array of 30 outfits:
[
  {
    "occasion": "casual|date|gala",
    "items": [{"id": "...", "type": "tops"}, {"id": "...", "type": "bottoms"}, ...],
    "description": "Brief outfit description (1 sentence)"
  }
]
...
Return ONLY the JSON array, no additional text.`;
        ...
        const response = await this.makeRequest([
            { role: 'system', content: 'You are a professional stylist creating capsule wardrobes and outfit collections.' },
            { role: 'user', content: prompt }
        ], {
            max_tokens: 4000,
            temperature: 0.8
        ...
        try {
            const cleanResponse = response.replace(/```json\n?|\n?```/g, '').trim();
            const outfits = JSON.parse(cleanResponse);

            // Add full item details to each outfit
            ...
            console.error('Failed to parse vision board outfits:', error);
            throw new Error('Failed to generate vision board outfits. Please try again.');
        }
    }
}
